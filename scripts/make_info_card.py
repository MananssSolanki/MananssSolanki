...
ROWS = [
    ("host",),
    ("kv", "Now", "Software Developer @ Norght Infotech"),
    ("kv", "Prev", "Mobile App Dev Intern @ Norght Infotech"),
    ("kv", "Edu", "MCA, Marwadi University '26"),
    ("kv", "Loc", "Rajkot, Gujarat, India"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Mobile", "Flutter, Dart, Kotlin, Java, Android SDK"),
    ("kv", "State", "GetX, Provider, Riverpod, BLoC"),
    ("kv", "Backend", "Node.js, Express, REST APIs, Firebase"),
    ("kv", "Data", "SQLite, MongoDB, Firestore"),
    ("kv", "Arch", "MVVM, Clean Architecture"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "2+ yrs shipping production Flutter/Android apps"),
    ("bul", "4 apps built · live on the Play Store"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">manan@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">manan</tspan><tspan fill="{MUTED}">@</tspan>'
